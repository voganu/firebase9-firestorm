...
describe("[unit] FirestoreSerializer", () => {
  ...
  describe("#serialize", () => {
    describe("serializing create request", () => {
      ...
      postData.body = "body";
      let serialized = FirestoreSerializer.serialize(
        postData,
        WriteTypes.Create
      );
      ...
      it("should produce array of document references", () => {
        ...
        const commentRef = Collection(Post)
          .doc("hello-world")
          .collection(Comment)
          .doc("comment-1");
        authorData.favoritedComments = [commentRef];
        const authorSerialized = FirestoreSerializer.serialize(
          authorData,
          WriteTypes.Create
        );
    ...
    describe("serializing update request", () => {
      ...
      it("should remove timestamp if not provided", () => {
        ...
        const serialized = FirestoreSerializer.serialize(
          postData,
          WriteTypes.Update
        );
      ...
      it("should include timestamp if provided and not set to auto-update", () => {
        ...
        expect(serialized.posted.isEqual(Timestamp.fromMillis(0).native)).to.be
          .true;
      });
    });
  });
