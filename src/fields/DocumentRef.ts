...
class DocumentRef<T extends Entity> implements IDocumentRef<T> {
  ...
  public get cached(): T | null {
    if (!this._cachedDocument) {
      throw new Error(
        "Can not fetch cached document reference, call get() first."
      );
    }
   ...
   */
  public get native(): DocumentReference {
    return this._native;
  }
   ...
   */
  private buildNative(): DocumentReference {
    return doc(this._parent.native, this._id);
  ...
  public get(): Promise<T> {
    return new Promise((resolve): void => {
      if (this._cachedDocument === null) {
        getDoc(this._native).then((snapshot): void => {
          this._cachedDocument = FirestoreSerializer.deserialize(
            snapshot,
            this._model,
            this.parent
          );
  ...
  public collection<C extends Entity>(
    ...
    const currentSubcollections = getRepository(
      this._model.prototype.constructor.name
    ).subcollections;
    const collectionExists = currentSubcollections.has(
      childRepository.collectionConfig.name
    );
    if (collectionExists) {
      return Collection(collectionModel, this);
    }
    throw new Error(
      `Could not find collection ${collectionModel.prototype.constructor.name} in parent ${this._model.prototype.constructor.name}`
    );
  }
