...
const serialize = (
  ...
  return FieldUtils.process(
    ...
    TimestampOrig | FieldValueOrig | undefined => {
      const isAutoWriteCondition =
        writeType === WriteTypes.Create && (updateOnWrite || updateOnCreate);
      const isAutoUpdateConfition =
        writeType === WriteTypes.Update && (updateOnWrite || updateOnUpdate);
...
const deserialize = (
  isArray: boolean,
  value: TimestampOrig | TimestampOrig[]
): Timestamp | Timestamp[] => {
  return FieldUtils.process(isArray, value, (v: TimestampOrig): Timestamp => {
...
const toData = (
  ...
  return FieldUtils.process(isArray, value, (v: Timestamp): string =>
    format(v.toDate())
  );
};
...
export default function (fieldConfig?: ITimestampConfig): Function {
  return function (target: any, key: string): void {
    ...
    field.format =
      _fieldConfig.format || ((date: Date): string => date.toLocaleString());
    field.deserialize = (
      value: TimestampOrig | TimestampOrig[]
    ): Timestamp | Timestamp[] => {
