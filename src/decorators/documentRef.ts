...
const serialize = (
  ...
  return FieldUtils.process(
    isArray,
    value,
    (v: IDocumentRef<IEntity>): DocumentReference => v.native
  );
};
/**
...
const deserialize = (
  ...
  const deserializeValue = (
  ...
  ): IDocumentRef<IEntity> => {
    let parentEntityName = entity.prototype.constructor.name;
    let firestoreParent: CollectionReference | null =
      firestoreDocRef.parent || null;
    ...
    return DocumentRef(
      firestoreDocRef.id,
      entity,
      buildCollectionTree(parentEntityName, firestoreParent)
    ) as any;
  };
  if (isArray) {
    return (value as DocumentReference[]).map(deserializeValue);
  } else {
    return deserializeValue(value as DocumentReference);
  }
};
...
const toData = (
  ...
  if (isArray) {
    return (value as IDocumentRef<Entity>[])
      .map(valueToData)
