  ...
  implements _IQuerySnapshot<T>
{
   ...
   * @hidden
   */
  private _nativeSnapshot: QuerySnapshotOrig;
  /**
   * @hidden
   */
  private _docs: T[];
  ...
  public get size(): number {
    return this._nativeSnapshot.size;
  }
  ...
  public get empty(): boolean {
    return this._nativeSnapshot.size === 0;
  }
  ...
  public docChanges(opts?: SnapshotListenOptions): DocumentChange<T>[] {
    const changes = this._nativeSnapshot
      .docChanges(opts)
      .map((change): DocumentChange<T> => {
   ...
   * @hidden
   */
  private deserializeValue = (nativeValue: DocumentSnapshot): T => {
