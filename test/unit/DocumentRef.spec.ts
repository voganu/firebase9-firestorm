...
import "mocha";
import { Collection, ICollection, IDocumentRef, DocumentRef } from "../../src";
  ...
  describe("#path", (): void => {
    ...
    it("document ref to nested document should produce valid path", async (): Promise<void> => {
      ...
      if (author) {
        ...
        expect(favoriteComment.path).to.equal(
          "/posts/hello-world/comments/comment-1"
        );
      }
    });
  });
