...
describe("[unit] FieldUtils", () => {
  ...
  describe("#configure()", () => {
    describe("field name is provided", () => {
      for (const fieldType in FieldTypes) {
        ...
        describe(`with field type ${fieldType}`, () => {
          it("name should be provided name", () => {
            expect(result.name).to.equal("TEST-FIELD-NAME");
          });
          it("type should match", () => {
            expect(result.type).to.equal(type);
          });
          it("should not be an array", () => {
            expect(result.isArray).to.equal(false);
          });
    ...
    describe("field name is determined by firestorm config", () => {
      const run = (
        conversationType: FieldConversionType,
        expectedNameVal: string
      ) => {
        bootstrap.start({
        ...
        it("name should have converted name value", () => {
          expect(result.name).to.equal(expectedNameVal);
        });
      };
      [
  ...
  describe("#process()", () => {
    ...
    it("running processor on an array should apply to all array values", () => {
      const result = FieldUtils.process(
        true,
        [1, 2],
